<ul class="brands-list {{ isset($modification) && $modification ? ' brands-list--' . $modification : '' }} {{ $classNameUl ?? '' }}"
    {{ $attributes ?? '' }}>
        ...
        <li
            ...
            <div class="brands-list__info">
                ...
                @if (isset($prices) && isset($categoryID) && isset($prices[$categoryID]['items'][$subCategory->id]))
                    <p class="brands-list__price">от {{ $prices[$categoryID]['items'][$subCategory->id]->price }} руб.
                    </p>
                @endif
            </div>
        </li>
